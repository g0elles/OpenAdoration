...
1. **Definition of Done (per milestone/feature).** Order doesn't matter; proof does. A change
   ...
   - `ROADMAP.md` (status) updated **in the same commit**.
...
3. **One source of truth per fact.** `ROADMAP.md` = milestone status. Tests = code invariants.
   Don't restate a fact in a second doc; link it.
...
- **Where each kind lives.** Policy/rules → `CLAUDE.md` (+ G-gotchas). Working preferences & durable
  facts → `.claude/memory/` (typed frontmatter) and `ROADMAP.md` (milestone status). The repo + git
  history is the raw trace. When a fact in a working note becomes durable, **promote it** to the right
  typed home and let the raw note age out.
...
- Active work → `dev` (public repo; commit/push here).
