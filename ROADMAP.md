...
- **Deferred (ponytail):** enable/disable toggle — `remove` covers "stop using it"; add a disabled-marker + restart only if a church wants to keep a plugin installed-but-inactive. Secret fields render as plain text in the UI, but at rest the settings blob is DPAPI-encrypted (`settings.dat`, CurrentUser scope) as of 2026-06-23 (audit S5) — no longer plaintext on disk.
...
| ~~**NU1903** — `SQLitePCLRaw.lib.e_sqlite3 2.1.11` advisory~~ | ✅ **Fixed 2026-06-23 (S2)** — direct `SQLitePCLRaw.bundle_e_sqlite3` 3.0.3 ref overrides EF's transitive 2.1.11; `--vulnerable` clean, 82/82 infra tests pass. **Owed:** `win-x64` publish/MSI smoke-test (native 2→3 bump). |
| **P2 — Song list/search eager-loads all `Sections`** | Perf: `SongRepository.GetAll/SearchByTitle/SearchByLyrics` all `Include(Sections)`, but list/search only need summary fields. Fix = drop the includes **and** re-fetch full song by id at `SongsViewModel.EditSong` + `ProjectSong` (naive drop blanks lyrics on save + empties projection slides — traced 2026-06-23). Scale-dependent Medium; needs a regression test. Logged from the 2026-06-23 audit (#P2). |
